...
- **Bedrock推論対応** - `bedrock/<model-id>` を指定して AWS Bedrock Runtime 経由で推論
...
Bedrock は `InvokeModel` と `InvokeModelWithResponseStream` の両方に対応している。`bedrock_include_reasoning: false` の場合は `<reasoning>...</reasoning>` を除去して返し、stream では空になった chunk を間引いたうえで終端に OpenAI 互換の `usage` chunk を追加する。
...
- `model_list`: `litellm_params.model` のプレフィックス (`openai/`, `gemini/`, `bedrock/`) でプロバイダ判定
- Bedrock 利用時は `litellm_params.region` を優先し、未指定なら `AWS_REGION` または `AWS_DEFAULT_REGION` を利用
