...
func forwardOpenAICompatEmbeddings(w http.ResponseWriter, r *http.Request, cfg *Config, logger *RequestLogger, req OpenAIEmbeddingRequest, provider, providerLabel, model, targetURL, authHeader string, bodyBytes, upstreamBody []byte, reqID string, start time.Time, timeout time.Duration) {
	upstreamCtx, cancel := withUpstreamTimeout(r.Context(), true, timeout)
	defer cancel()
...
func handleOpenAIEmbeddings(w http.ResponseWriter, r *http.Request, cfg *Config, logger *RequestLogger, req OpenAIEmbeddingRequest, model string, bodyBytes []byte, reqID string, start time.Time, perModelCfg ModelConfig) {
	...
	forwardOpenAICompatEmbeddings(w, r, cfg, logger, req, "openai", "openai", model, targetURL, "Bearer "+apiKey, bodyBytes, modifiedBody, reqID, start, perModelCfg.Timeout)
}
...
func handleGeminiEmbeddings(w http.ResponseWriter, r *http.Request, cfg *Config, logger *RequestLogger, req OpenAIEmbeddingRequest, model string, bodyBytes []byte, reqID string, start time.Time, perModelCfg ModelConfig) {
	...
	if len(inputs) == 1 {
		...
		upstreamCtx, cancel := withUpstreamTimeout(r.Context(), true, perModelCfg.Timeout)
		defer cancel()
...
func handleOllamaEmbeddings(w http.ResponseWriter, r *http.Request, cfg *Config, logger *RequestLogger, req OpenAIEmbeddingRequest, model string, bodyBytes []byte, reqID string, start time.Time, perModelCfg ModelConfig) {
	...
	forwardOpenAICompatEmbeddings(w, r, cfg, logger, req, "ollama_chat", "ollama", model, targetURL, authHeader, bodyBytes, modifiedBody, reqID, start, perModelCfg.Timeout)
}
...
func handleOpenRouterEmbeddings(w http.ResponseWriter, r *http.Request, cfg *Config, logger *RequestLogger, req OpenAIEmbeddingRequest, model string, bodyBytes []byte, reqID string, start time.Time, perModelCfg ModelConfig) {
	...
	forwardOpenAICompatEmbeddings(w, r, cfg, logger, req, "openrouter", "openrouter", model, targetURL, "Bearer "+apiKey, bodyBytes, modifiedBody, reqID, start, perModelCfg.Timeout)
}
